...
import { LuAlignJustify, LuListVideo, LuMoreVertical, LuTrash2 } from 'react-icons/lu';
...
export const DesktopQueueItem = ({ video, isPlaying, isCurrent, isDragging, onRemove, onPlay, isPlaylistItem }: Props) => {
  ...
  return (
    ...
    >
      <Box position='relative' flexShrink={0}>
        <Image width='120px' minHeight='100%' objectFit='cover' borderRadius='sm' aspectRatio={'16/9'} src={video.thumbnail} />
        {isPlaying && (
        ...
        <Icon as={BsFillPlayFill} className='queue-item-play-icon' boxSize={7} />

        {video.live && <LivestreamTag />}
        {!video.isPlaylist && isCurrent && <QueueItemProgressBar duration={video.duration} />}
      ...
      <Flex direction='column' gap={1}>
        ...
        <Flex alignItems='center' gap={2}>
          {video.channelThumbnail && <Image borderRadius='full' width='25px' objectFit='cover' aspectRatio={'1/1'} src={video.channelThumbnail} />}
        ...
        <Flex alignItems='center' gap={1} wrap='wrap' fontSize='xs' color='text.300'>
          ...
          {video.isPlaylist && (
            <>
              <Tag as='span' size='sm' fontWeight='bold' variant='subtle' display='inline-flex' bg='bg.400'>
